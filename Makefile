...
build:
	...
	docker run --rm -v ${DIR}/src:/go/src/skaioskit -w /go/src/skaioskit golang:latest go build -ldflags "-linkmode external -extldflags -static" -o auth

clean:
	rm ${DIR}/src/auth

pack:
	docker build -f ./Dockerfile -t localhost:5000/skaioskit/auth-service .

upload:
	docker push localhost:5000/skaioskit/auth-service
